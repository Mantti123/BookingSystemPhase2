...
function isResourceAvailableValid() {
  return document.getElementById("resourceAvailable")?.checked === true;
}
...
function isFormValid() {
  ...
  return (
    isResourceNameValid(name) &&
    isResourceDescriptionValid(desc) &&
    isResourcePriceValid(price) &&
    isResourceAvailableValid()
  );
}
...
function attachResourceNameValidation(input) {
  const update = () => {
    ...
    // Create enabled only if whole form is valid
...
function attachFormValidationListeners() {
  const fields = [
    ...
    document.getElementById("resourceAvailable"),
...
// Attach validation for description
...
// Attach validation to the rest of the form
